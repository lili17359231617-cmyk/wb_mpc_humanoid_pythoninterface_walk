...
- **学得明白**：在默认或轻度调参下，训练曲线出现明显学习信号（episode 回报上升、episode 长度增加、跌倒率下降）。
...
| 7 | 轻量随机化（可选） | 在 `G1MpcWeightEnv.reset` 中对初始高度/姿态或 `velocity_command` 做小范围随机，提升鲁棒性 | 策略从小适应不确定性，为 Phase 2 打基础 |
...
**核心目标**：在 Phase 1 策略与底层均稳定的前提下，通过**大量并行 env** 提升样本效率与训练吞吐。
...
1. **跑一次短训练**（约 1e5 steps），保存 TensorBoard 或曲线数据，观察 reward 与 episode length。
2. **根据曲线调整** `MpcWeightEnvReward` 与 `fall_height_threshold`，直到单 env 上能明显「学得动」。
3. **（可选）** 在 `G1MpcWeightEnv.reset` 中加入轻度状态/指令随机化，观察训练稳定性。
4. **启用 4～8 个并行 env**，确认稳定性与资源占用后，将第一阶段视为完成，再规划第二阶段的具体排期与实现细节。
