"""
G1 MPC 权重 PPO 训练脚本（行走导向 / Phase 2）

使用包内配置与环境：默认使用 G1MpcWeightEnv（仿真同步多速率），
也可切换为 G1MpcEnv（简化 MuJoCo 步进）。路径从 humanoid_wb_mpc.config 读取。

- 续训加载（LOAD_MODEL）：使用 PPO.load(..., custom_objects={"action_space": env.action_space, "observation_space": env.observation_space})
  传入当前 env 的 space，以跳过 zip 内与当前 env 的 space 一致性检查，避免因 action/obs 范围
  或 shape 微调导致加载失败。
...
# ─── 两阶段 active Q 维定义 ───────────────────────────────────────────────────
# MPC 状态布局（58维）：base_pos[0:3], base_euler[3:6], joint_pos[6:29],
#                       base_vel[29:32], euler_dot[32:35], joint_vel[35:58]
# Phase1：只调 base 相关维（高度、roll/pitch、base 线/角速度）
PHASE1_ACTIVE_DIMS = [2, 4, 5, 29, 30, 31, 32, 33, 34]
# Phase2：Phase1 基础上加腿部关节 pos 维（左腿 6-11，右腿 12-17）
PHASE2_ACTIVE_DIMS = PHASE1_ACTIVE_DIMS + list(range(6, 18))
...
def main():
    ...
    load_model_path = os.environ.get("LOAD_MODEL", "").strip()

    if phase == 1:
        # Phase 1：纯站立鲁棒性，只调 base 相关 Q 维，强 domain rand
        phase_active_dims = PHASE1_ACTIVE_DIMS
        phase_vel_cmd = np.array([0.0, 0.0, 0.75, 0.0])
        phase_force_prob = float(os.environ.get("FORCE_PROB", "0.70"))
        phase_force_mag = (
            float(os.environ.get("FORCE_MAG_MIN", "30.0")),
        ...
        # Phase 2：行走稳定性，调 base + 腿部 Q 维，速度随机化，加步态对称奖励
        phase_active_dims = PHASE2_ACTIVE_DIMS
        phase_vel_cmd = np.array([0.15, 0.0, 0.75, 0.0])   # 基础前进速度
        phase_force_prob = float(os.environ.get("FORCE_PROB", "0.45"))
        phase_force_mag = (
            float(os.environ.get("FORCE_MAG_MIN", "28.0")),
    ...
    print(f"🚀 启动训练: G1 步态残差优化（Phase {phase}）...")
    ...
    try:
        ...
        print("   → 请先用单 env 复现以查看真实异常: N_ENVS=1 ... python3 train_g1_ppo_v1.py")
        print("   → 若已修改 C++ 绑定，请先重新编译: colcon build --packages-select humanoid_wb_mpc && source install/setup.bash")
        raise SystemExit(1) from e
    save_name = f"g1_ppo_residual_v1_phase{phase}_{total_steps}" + (f"_n{n_envs}" if n_envs > 1 else "")
